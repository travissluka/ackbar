...
def test_the_script_is_a_pure_function_of_the_node(paths):
    ...
    # timestamp satisfies them and they still agree.
    assert text(paths, ARRAY) == text(paths, ARRAY)

    # Nor does moving `time.time`, which was the first repair here and covered
    # the less likely spelling: `datetime.now()` is C and reads the OS clock
    # directly, so `mock.patch("time.time")` cannot reach it, and a script
    # generator is likelier to reach for `datetime` than for `time`.
    #
    # The golden is what actually holds the claim. It catches any clock, any
    # spelling of one, and a filesystem read too, without anyone having to guess
    # which API a future edit reaches for. The only varying part is the tmpdir,
    # so it normalizes to one token. When emit changes on purpose this fails and
    # the diff is the review.
    assert text(paths, ARRAY).replace(str(paths.output_root.parent), "<TMP>") == (
        "#!/bin/bash\n"
        "#SBATCH --job-name=e.2.forecast\n"
        "#SBATCH --comment=ackbar:e:2:forecast\n"
        "#SBATCH --output=<TMP>/o/e/run/20180416T000000Z/log/forecast.%A_%a.out\n"
        "#SBATCH --open-mode=append\n"
        "#SBATCH --ntasks=8\n"
        "#SBATCH --time=00:30:00\n"
        "#SBATCH --mem=16G\n"
        "\n"
        "# forecast: generated by ackbar, do not edit.\n"
        "export ACKBAR_ROOT=/repo\n"
        'source "$ACKBAR_ROOT/site/activate.sh"\n'
        "set -euo pipefail\n"
        "ulimit -s unlimited || true\n"
        "\n"
        'exec "/venv/bin/python" -m ackbar.cli run e \\\n'
        "    --cycle 2 \\\n"
        "    --task forecast \\\n"
